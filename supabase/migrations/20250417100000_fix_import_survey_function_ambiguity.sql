-- Migration: fix_import_survey_function_ambiguity
-- Descripción: Corrige la ambigüedad de la variable section_order en la función import_survey_from_json

-- Eliminar la función existente para evitar conflictos
DROP FUNCTION IF EXISTS public.import_survey_from_json(jsonb);

-- Crear la función para importar encuestas desde JSON con correcciones para evitar ambigüedad
...
BEGIN
    ...
    IF survey_data -> 'sections' IS NOT NULL THEN
        ...
        LOOP
            ...
            RAISE NOTICE 'Insertando sección: % (Orden: %)', section_title, v_section_order;
            ...
            -- Usando la variable renombrada para evitar ambigüedad
            SELECT id INTO new_section_id 
            FROM public.survey_sections 
            WHERE survey_id = new_survey_id AND section_order = v_section_order
            ORDER BY id DESC
            LIMIT 1;
