-- Migration: 20250416225600_create_import_survey_function.sql
-- Descripción: Crea una función para importar encuestas desde un objeto JSON

-- Crear la función para importar encuestas desde JSON
-- Esta función permite importar una encuesta completa con sus secciones y preguntas desde un objeto JSON
-- Parámetros:
--   survey_data: Objeto JSONB con la estructura de la encuesta, secciones y preguntas
-- Retorna:
--   Un objeto JSONB con {success: true, surveyId: ID} si la operación fue exitosa
--   o {success: false, error: {code: string, message: string}} si hubo un error
--
...
--       "description": "Evalúe su experiencia con nuestro producto",
--       "section_order": 1,
--       "discount_percentage_cumulative": 10,
--       "questions": [
--         {
--           "question_text": "¿Cómo calificaría la calidad del producto?",
--           "question_type": "scale",
--           "options": [{"value": "1", "label": "Muy mala"}, {"value": "2", "label": "Mala"}, {"value": "3", "label": "Regular"}, {"value": "4", "label": "Buena"}, {"value": "5", "label": "Excelente"}],
--           "is_required": true,
--           "question_order": 0
--         }
--       ]
...
-- Ejemplo de uso:
-- SELECT public.import_survey_from_json('{"survey":{"name":"Mi Encuesta","slug":"mi-encuesta","description":"Descripción","requires_product_code":false,"is_active":true,"discount_code_template":"DESC{PERCENT}-{CODE}","discount_code_validity_days":30},"sections":[{"title":"Sección 1","description":"Primera sección","section_order":0,"discount_percentage_cumulative":5,"questions":[{"question_text":"Pregunta 1","question_type":"text_short","is_required":true,"question_order":0}]}]}');
